...
public class Lab4 extends GraphicsLab
{
    ...
    {   new Lab4().run(WINDOWED,"Lab 4 - Lighting",0.05f);
    }
    ...
    {
    	if(Keyboard.isKeyDown(Keyboard.KEY_RSHIFT) && Keyboard.isKeyDown(Keyboard.KEY_R) )
        {   houseRotationAngle -= 1.0f * getAnimationScale(); // Make the house go around if the R key is pressed
            if (houseRotationAngle < 0.0f) // Wrap the angle back around into 0-360 degrees.
    ...
    protected void renderScene()
    {
     ...
     // how shiny are the front faces of the house (specular exponent)
        ...
        // specular reflection of the front faces of the house
        //float houseFrontSpecular[] = {0.1f, 0.0f, 0.0f, 1.0f};
        float doorFrontSpecular[] = {0.9f, 0.1f, 0.1f, 1.0f};
        // diffuse reflection of the front faces of the house
        //float houseFrontDiffuse[]  = {0.6f, 0.2f, 0.2f, 1.0f};
        float doorFrontDiffuse[]  = {0.9f, 0.1f, 0.1f, 1.0f};
        ...
        GL11.glTranslatef(-0.1f, -1.5f, 0.5f);
    ...
    private void drawUnitDoor() {
       	Vertex v1 = new Vertex(-0.2f, 0.15f, 0.1f); //top left
    	Vertex v2 = new Vertex(0.1f, 0.15f, 0.1f); //top right
    	
    	Vertex v3 = new Vertex(0.1f, -0.5f, 0.1f); //bottom right
    	Vertex v4 = new Vertex(-0.2f, -0.5f, 0.1f); //bottom left
    	
    	Vertex v5 = new Vertex(-0.2f, -0.5f, -0.1f); //back bottom left
    	Vertex v6 = new Vertex(0.1f, -0.5f, -0.1f); //back bottom right
    	
    	Vertex v7 = new Vertex(0.1f, 0.15f, -0.1f); //back top right
    	Vertex v8 = new Vertex(-0.2f, 0.15f, -0.1f); //back top left
    	...
    	GL11.glBegin(GL11.GL_POLYGON);
    	{
    		new Normal(v1.toVector(),v8.toVector(),v5.toVector(),v4.toVector()).submit();
    		//Colour.BLUE.submit();
    		v1.submit();
    		v8.submit();
    		v5.submit();
    	...
    	GL11.glBegin(GL11.GL_POLYGON);
    	{
    		new Normal(v3.toVector(),v4.toVector(),v5.toVector(),v6.toVector()).submit();
    		//Colour.BLUE.submit();
    		v3.submit();
    		v4.submit();
    		v5.submit();
